...
5. Show the clause inspector and consensus result. Call out the distinct transaction lifecycle, GenVM execution result, validator/finality details, and explorer link.
6. Open the receipt. Explain that the model supplies a normalized outcome and agent responsibility roles; citation differences do not change the agreement key, and contract code computes rewards, refunds, and bond effects from the sealed charter.
7. On **Remediation**, point out that no role is assigned and no accounting moves while the next attempt is opened. Switch on **Plain numbers** to replace the mascots and bars with text and a responsibility table.

The public review does not require a wallet. A wallet is needed only to submit transactions.
