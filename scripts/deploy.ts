...
import {
  ...
  checkNetwork, ensureFunded, fixtureBaseUrl, loadOrCreateKey,
...
async function main(): Promise<void> {
  ...
  console.log(`Checking Studio Net ${RPC_URL} (chain ${CHAIN_ID})…`);
  await checkNetwork();
  ...
  const schema = await withRetry("compile Studio Net Faultline schema", () => reader.getContractSchemaForCode(source));
  const methods = Object.keys(schema.methods);
  if (!methods.includes("get_contract_version") || methods.length !== 14) {
    throw new Error(`Studio Net compiled an unexpected Faultline schema (${methods.length} methods); refusing deployment.`);
  }
  console.log(`Stable Studio Net accepted the pinned runner and contract schema (${methods.length} methods).`);
  const balance = await ensureFunded(account.address);
  console.log(`Deployer ${account.address} has ${balance} wei available.`);
  const hash = await withRetry("deploy Faultline", () => client.deployContract({ code: source, args: [] })) as Hex;
