...
import {
  CHAIN_ID, DEPLOYMENT_FILE, FEE_PROFILE_FILE, PROOF_FILE, RPC_URL, checkNetwork, readJson, writeJson,
...
async function main(): Promise<void> {
  ...
  for (const observation of hashes) {
    ...
    if (lifecycle !== "FINALIZED" || !isSuccessful(transaction)) throw new Error(`${observation.name} is ${lifecycle} / ${transaction.txExecutionResultName ?? "unknown"}; only successful finalized transactions can be profiled.`);
    const distribution = extractDistribution(transaction);
    if (!distribution) throw new Error(`${observation.name} has no finalized fee accounting fields; this Studio Next response does not support empirical fee profiling.`);
  ...
  const profile = { version: 1, chainId: CHAIN_ID, network: "studio-dev", measuredAt: new Date().toISOString(), deploy, methods, observations };
  writeJson(FEE_PROFILE_FILE, profile);
  console.log(`Wrote fee allocations measured from ${observations.length} finalized Studio Next receipts.`);
